...
def mouse_move():
    ...
    while running:
        ...
        # read hedef_x/y safely
        with lock:
            tx, ty = hedef_x, hedef_y

        # temporal buffer
        buf_x.append(tx)
        buf_y.append(ty)
        smoothed_x = sum(buf_x) / len(buf_x)
        smoothed_y = sum(buf_y) / len(buf_y)
        ...
        dx = smoothed_x - onceki_x
        dy = smoothed_y - onceki_y
        mesafe = math.hypot(dx, dy)

        # Hız
        hiz = min(mesafe / max(dt, 0.001), max_speed)
        hiz_orani = hiz / max_speed

        # Dinamik deadzone
        deadzone = base_deadzone * (1 - hiz_orani)
        ...
        if mesafe > deadzone:
            ...
            if move_len > max_frame_move:
                oran = max_frame_move / move_len
                move_x *= oran * 1.3
                move_y *= oran * 1.3

            guncel_x = onceki_x + move_x
            guncel_y = onceki_y + move_y

            pyautogui.moveTo(guncel_x, guncel_y)
            onceki_x, onceki_y = guncel_x, guncel_y
